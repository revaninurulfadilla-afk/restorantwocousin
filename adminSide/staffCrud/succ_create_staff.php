...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        $sql = "INSERT INTO login (id_user, nama, email, username, password, role, no_hp, status)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

        $stmt = $conn->prepare($sql);
        $stmt->bind_param("isssssss", $id_user, $nama, $email, $username, $password, $role, $no_hp, $status);

        if (!$stmt->execute()) {
            throw new Exception("Error insert login: " . $stmt->error);
        }

        $conn->commit();

        $message = "Staff berhasil dibuat & bisa login!";
